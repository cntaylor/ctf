...
class Griewank(Function2D):
    ...
    def __init__(self, n):
        ...
        self.value = 0.0
        self.domain = np.array([[-600.0, 600.0] for i in range(0, n)])
        self.n = n
        self.smooth = True
    ...
    def cost(self, x):
        ...
        # Calculate Cost
        c = np.sum([x[i]**2/4000 for i in range(0, self.n)]) - np.prod([np.cos(x[i]/np.sqrt(i+1)) for i in range(0, self.n)]) + 1
        # Return Cost
        return c
